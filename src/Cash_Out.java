...
import java.awt.*;
public class Cash_Out implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btn) {
		 String sql = "SELECT `Agent_ID` , `F_name` FROM `agent` WHERE `Agent_ID` =  '"+agentID+"'";
	     ResultSet rs = stmt.executeQuery(sql);
       if(agentID.equals("")) {
			 JOptionPane.showMessageDialog(frame, "Agent_ID is empty ");
		 }else if (!rs.next()) {
//		        	        String notfound_acc = rs.getString("Account_Number");
   	        JOptionPane.showMessageDialog(frame, "Agent not found");
   	    }else {
   	    	   String id= rs.getString("F_name");
   	    	txt2.setText(id);
           ...
           try {
           	String sqlup = "SELECT * FROM `agent` WHERE `Agent_ID` = '" + agentID + "'";
		         ResultSet Nrs = stmt.executeQuery(sqlup);
	            if (Nrs.next()) {
	            	String Account = Nrs.getString("Agent_ID");
	                ...
	                int New_A = Integer.valueOf(amount);
	                	...
	                	JOptionPane.showMessageDialog(frame, "Your amount is not sufficient");
	 	                ...
	 	                sqlup = " UPDATE `agent` SET `Balance` = '" + New_Amount + "' WHERE `Agent_ID` = '" + agentID + "' ";
	 	                JOptionPane.showMessageDialog(frame, "Cash withdraw request is under process by the agent!");
